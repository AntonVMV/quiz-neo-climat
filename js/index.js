...
(function () {
  ...
  [...formSteps].forEach((item, index, arr) => {
    ...
    answers.addEventListener("change", (e) => {
      const selectedElement = [...answers.children].indexOf(
        e.target.parentNode
      );
  ...
  //Show calculating
  function showCalculating(index) {
    const plug = document.querySelector(".calculatin-plug");

    console.log(formSteps[index]);
    waitForAnimEnds(formSteps[index], plug);

    setTimeout(() => {
      waitForAnimEnds(plug, formSteps[index + 1]);
    }, 2000);
  }
